...
2.  **本專案對策**：使用 `better-sqlite3` 同步模式 + WAL (Write-Ahead Logging) 模式，已大幅優化此問題，足以應付一般中小企業倉儲流量。
3.  **缺乏使用者權限管理**：沒有內建的使用者帳號系統 (如 `GRANT SELECT TO user`)，權限需由應用程式層 (Node.js) 控制。
4.  **資料量限制**：雖然理論支援 140TB，但當單一檔案超過 10GB 時，備份與維護會變得較困難。建議適合 1GB 以下的專案。
