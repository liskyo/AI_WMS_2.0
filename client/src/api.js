...
export const importInventory = (inventory, token, options = {}) => api.post('/admin/import/inventory', { inventory, ...options }, { headers: { Authorization: `Bearer ${token}` } });
export const getInventoryCount = (token) => api.get('/inventory/count', { headers: { Authorization: `Bearer ${token}` } });
